...
This directory contains the SoC-specific part of the uberClock repository:

- ``1.hw`` for active SoC RTL and the ``testing`` area
- ``2.sw`` for firmware / C-side control code
- ``5.docker`` for containerized setup support
- ``6.migen`` for the legacy LiteX integration path
- ``7.gui`` for host-side GUI experiments
- ``8.python`` for the current packaged Python/LiteX integration layer

The repo-level build workspace now lives at ``3.build`` instead of inside this
directory.

Legacy architecture notes retained below:
