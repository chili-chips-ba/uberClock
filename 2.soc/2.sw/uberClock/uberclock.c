...
/**
 * @file uberclock.c
 * @brief UART-driven bare-metal firmware for the uberClock LiteX SoC.
 *
 * This file contains the current firmware runtime used by the VexRiscV CPU.
 * It registers UART console commands, writes LiteX-generated CSRs, services the
 * `ce_down` event from the DSP datapath, moves samples through the low-rate
 * FIFOs, runs KISS FFT analysis, starts DDR/S2MM captures, and implements the
 * `track3` and `trackq` frequency-tracking workflows.
 *
 * The application directory is generated by `demo.py`, so this top-level source
 * in `2.soc/2.sw` is the copy that should be documented and edited.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
/**
 * @brief Write one signed X sample into every CPU-driven upsampler input CSR.
 *
 * The current console commands treat CPU injection as a five-channel frame.
 * Hardware variants may expose individual channel CSRs or a replicated scalar
 * path; this helper is the single place that fans out the requested X value.
 */
...
/**
 * @brief Write one signed Y sample into every CPU-driven upsampler input CSR.
 */
...
/**
 * @brief Push a five-channel IQ frame into the upsampler injection FIFO.
 *
 * The frame is presented through CSR storage registers and committed with the
 * `main_ups_fifo_push` strobe. Samples are stored as 16-bit signed values.
 */
...
/**
 * @brief Push the same IQ sample into all five logical upsampler channels.
 */
...
/**
 * @brief Pop one downsampled IQ frame from the DSP-to-CPU FIFO.
 *
 * The pop strobe latches the FIFO output into the generated CSR status fields.
 * Callers must check `main_ds_fifo_flags` before using this helper.
 */
...
/**
 * @brief Console command: run a fixed 32-point real FFT over DS FIFO Y samples.
 */
...
/**
 * @brief Convert a signal-generator frequency to a 32-bit DDS phase increment.
 */
...
/**
 * @brief Recompute all software three-tone DDS phase increments.
 */
...
/**
 * @brief Set the three software tone frequencies for one logical channel.
 */
...
/**
 * @brief Enable the software three-tone generator and prime its increments.
 */
...
/**
 * @brief Disable the software three-tone generator and push a zero sample.
 */
...
/**
 * @brief Advance all enabled three-tone DDS channels by one sample.
 *
 * @return Non-zero when at least one channel is enabled and `frame` was filled.
 */
...
/**
 * @brief Generate and enqueue one software signal-generator frame if enabled.
 */
...
/**
 * @brief Flush CPU/L2 caches before or after DMA-visible DDR accesses.
 */
...
/**
 * @brief Convert an RF/sample frequency to the 26-bit hardware phase format.
 */
...
/**
 * @brief Convert a 26-bit hardware phase increment back to Hz.
 */
...
/**
 * @brief Return squared magnitude for FFT bin `k` from the global FFT output.
 */
...
/**
 * @brief Service one low-rate `ce_down` event from the DSP datapath.
 *
 * This is the runtime hook that keeps software signal generation, magnitude
 * polling, and event re-arming aligned to the downsampled DSP cadence.
 */
...
/**
 * @brief Drain up to `max_samples` entries from the downsample FIFO.
 */
...
/**
 * @brief Capture `n` downsampled X samples for one logical channel into FFT input.
 */
...
/**
 * @brief Capture synchronized samples for the three-channel tracking loop.
 */
...
/**
 * @brief Estimate FFT-bin power at a requested frequency using live FFT output.
 */
...
/**
 * @brief Run an FFT over a sample buffer and return power near `f_hz`.
 */
...
/**
 * @brief Run one non-blocking iteration of the quadratic three-point tracker.
 *
 * When enabled by `trackq_start`, this function captures probe samples,
 * compares left/center/right tone power, and adjusts downconversion phase
 * increments with bounded correction steps.
 */
...
/**
 * @brief Parse an unsigned console argument with an inclusive upper bound.
 */
static inline unsigned parse_u(const char *s, unsigned max, const char *what) {
...
/**
 * @brief Parse a signed console argument with inclusive bounds.
 */
static inline int parse_s(const char *s, int minv, int maxv, const char *what) {
...
/* ---- COMMIT helper ---- */
/**
 * @brief Commit pending CSR settings to the UC-domain snapshot FIFO.
 */
static inline void uc_commit(void) {
...
/* ---- ISR ---- */
/**
 * @brief Interrupt handler for the DSP downsample event source.
 *
 * The ISR keeps work minimal: it acknowledges the event and increments a
 * software counter consumed by `uberclock_poll`.
 */
...
/* ---- Help ---- */
/**
 * @brief Console command: print all registered uberClock commands.
 */
...
/**
 * @brief Move a bounded number of samples between software queues and FIFOs.
 *
 * This non-blocking pump lets the firmware exercise the DSP loop without
 * monopolizing the UART console.
 */
static unsigned dsp_pump_step(unsigned max_in, unsigned max_out) {
...
/**
 * @brief Clear sticky overflow/underflow flags on DSP-side software FIFOs.
 */
...
/**
 * @brief Capture DS FIFO IQ samples and run a configurable KISS FFT.
 *
 * @param args Console argument string containing optional FFT length.
 * @param peak_only Print only the strongest bin when non-zero.
 */
...
/**
 * @brief Convert a byte-count token into the S2MM transfer-size encoding.
 */
...
/**
 * @brief Program and start an UberDDR3 S2MM DMA transfer.
 */
...
/**
 * @brief Store a 32-bit word in little-endian byte order for UDP payloads.
 */
...
/**
 * @brief Parse dotted-decimal IPv4 text into the LiteEth integer format.
 */
...
/**
 * @brief Register all uberClock console commands with the UART console.
 */
...
/**
 * @brief Initialize the experimental magnitude-search state machine.
 */
void fsm_init(void) {
 curr_state = IDLE; 
 ce_ticks = 0;
...
/**
 * @brief Advance the experimental magnitude-search state machine by one step.
 */
void tran(void) {
    switch (curr_state) {
        case IDLE: {
...
/**
 * @brief Initialize CSRs, software generators, FIFOs, interrupts, and defaults.
 *
 * This function is called once from the firmware entry point after the console
 * is ready. It establishes the default downconversion frequencies, routing,
 * gains, capture mode, and event interrupt state used by the UART workflows.
 */
...
/**
 * @brief Run the non-blocking firmware background work.
 *
 * The main loop calls this repeatedly after `console_poll`. It services the
 * quadratic tracker and drains queued `ce_down` events recorded by the ISR.
 */
