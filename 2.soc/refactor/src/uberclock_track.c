...
static int32_t estimate_side_error_mhz(uint64_t left_power, uint64_t right_power, uint32_t delta_hz) {
    ...
    magnitude = (difference < 0) ? (uint64_t)(-difference) : (uint64_t)difference;
...
static int validate_tracking_request(const char *name, unsigned sample_count, uint32_t center_hz, uint32_t delta_hz) {
...
void uberclock_track_poll(void) {
    ...
    if (vertex_is_confident(left_power, center_power, right_power)) {
        ...
        if (error_mhz > 0) {
            ...
            (track->filtered_error_mhz * (UBERCLOCK_TRACK_ERR_ALPHA_DEN - UBERCLOCK_TRACK_ERR_ALPHA_NUM)) /
            UBERCLOCK_TRACK_ERR_ALPHA_DEN;
            mode = "weak";
        }
    }

    filter_delta_mhz = ((int64_t)(error_mhz - track->filtered_error_mhz) * (int64_t)UBERCLOCK_TRACK_ERR_ALPHA_NUM) /
    (int64_t)UBERCLOCK_TRACK_ERR_ALPHA_DEN;
    track->filtered_error_mhz += (int32_t)filter_delta_mhz;

    control_mhz = ((int64_t)track->filtered_error_mhz * (int64_t)UBERCLOCK_TRACK_KP_NUM) /
    ...
    correction_hz = weak_mode ? clamp_weak_tracking_step_hz(correction_hz) : clamp_tracking_step_hz(correction_hz);
...
int uberclock_track3_run(uint32_t start_hz,
    ...
    original_phase_inc = uberclock_channel_get_phase_down(0u);
    sweep_hz = start_hz;
    ...
    for (step_index = 0u; step_index < max_steps; ++step_index) {
        ...
        if (!uberclock_fft_capture_ds_ch1_real(sample_count, UBERCLOCK_TRACK_DEFAULT_SETTLE)) {
            (void)uberclock_channel_set_phase_down(0u, original_phase_inc);
            uberclock_commit_config();
            return -1;
        }
        ...
        uberclock_runtime_wait_ticks(UBERCLOCK_TRACK_INTERVAL_TICKS);
        sweep_hz += step_hz;
    }
    ...
    printf("track3 no lock found in %u steps; restored phase_down_1=%lu Hz\n",
           max_steps,
           (unsigned long)uberclock_phase_inc_to_hz(original_phase_inc, UBERCLOCK_TRACK_RF_FS_HZ));
    return -1;
                         }
