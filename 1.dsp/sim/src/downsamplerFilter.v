module downsamplerFilter (
    ...
    // Intermediate wires
    wire signed [15:0] cic_out,decim_clk;
    wire               cic_ce_out;
...
//	);
    ...
    cic_dec #(
        .I_WIDTH   (12),      // input sample width
        .O_WIDTH   (12),      // output sample width
        .RMAX      (1625),    // maximum decimation rate
        .M         (1),       // differential delay
        .N         (4)       // number of stages
        //.REG_WIDTH (56)      // accumulator width
    ) cic_dec_inst (
        .clk          (clk),          // system clock
        .rst          (reset),          // active-high reset
        .clk_enable   (clk_enable),   // global clock enable
        .input_tdata  (filter_in),   // input sample (I_WIDTH bits)
        .output_tdata (cic_out),    // decimated sample (O_WIDTH bits)
        .clk_out      (decim_clk),    // gated clock at decimated rate
        .ce_out       (cic_ce_out)  // one-cycle pulse at decimated rate
    );
