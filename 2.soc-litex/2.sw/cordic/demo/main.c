...
static void phase_cmd(char *args) {
	...
	/* 1) write the new angle */
	main_cordic_phase_write(p);
	printf("Phase set to %u\n", p);

	/* 2) wait up to N cycles for valid flag */
	int timeout = 1000;
	while (!main_cordic_aux_read() && timeout--) {
		/* spin */
	}
	if (timeout <= 0) {
		printf("Warning: CORDIC valid timeout\n");
	}

	/* 3) read back the 12-bit result */
	unsigned y = main_cordic_y_read();
	printf("CORDIC Y = %u\n", y);

	#ifdef CSR_LEDS_BASE
	/* 4) drive LEDs: bits [11:9] of y, plus valid in bit 3 */
	unsigned leds = ((y >> 9) & 0x7)
	| ((main_cordic_aux_read() & 1) << 3);
	leds_out_write(leds);
	printf("LEDs = 0x%X\n", leds);
	#endif
}
